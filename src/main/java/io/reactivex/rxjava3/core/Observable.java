...
public abstract class Observable<@NonNull T> implements ObservableSource<T> {
    ...
    public static <T> Observable<T> create(@NonNull ObservableOnSubscribe<T> source) {
        ...
        //1.首先将我们的source包装起来,返回一个真正的Observable对象ObservableCreate
        //2.尝试对原始ObservableCreate对象进行hook,并返回一个新Observable
    ...
    /**
     * 返回一个新的Observable,用于在指定的线程执行订阅者的方法
     *
    ...
    /**
     * 订阅事件,并开始执行订阅者方法,订阅者方法会发送事件给我们的observer
    ...
    public final void subscribe(@NonNull Observer<? super T> observer) {
        Objects.requireNonNull(observer, "observer is null");
        try {
            //hook观察者的onSubscribe方法
            observer = RxJavaPlugins.onSubscribe(this, observer);

            //hook后,必须返回一个非空的新onSubscribe
